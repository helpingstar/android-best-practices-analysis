...
```
You are to analyze the provided image and determine if it is acceptable and appropriate based on specific criteria.
...
```
https://services.google.com/fh/files/misc/android_dancing.gif
```
...
```
Extract detailed information from the provided image. THE GOAL is to using this information to recreate the image with image generation model.
...
* Do not include brand logos or brand names in your descriptions.
* Do not include descriptions of emblems present.
...
```
Create a new image description where the subject(s) and surroundings are transformed into subject(s) attributes  hair color, clothing, accessories are important for human subjects.
...
```
https://services.google.com/fh/files/misc/androidfy_storyboard_b_v07.mp4
```
